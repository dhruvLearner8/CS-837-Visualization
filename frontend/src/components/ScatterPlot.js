...
//   .attr('class', 'grid')
//   .call(d3.axisLeft(y)
//     .tickValues(yTickValues)        // ✅ Only major ticks
//     .tickSize(-innerWidth)
...
const ScatterPlot = ({
  ...
  useEffect(() => {
    ...
    const originalX = d3.scaleLog()
      .domain([
        50,
        d3.max(filteredData, d => d.gdp_per_capita) || 100000
      ])
      .range([0, innerWidth]);
    ...
    const originalY = d3.scaleLog()
      .domain([
        0.02,
        d3.max(filteredData, d => d.co2_emissions_per_capita) || 10
      ])
      .range([innerHeight, 0]);
    ...
    // Safe tick values
    const yTickValues = [0.1, 1, 5, 10, 30].filter(val => val <= y.domain()[1]);
    const xTickValues = [50, 100, 1_000, 5_000, 20_000, 100_000].filter(val => val <= x.domain()[1]);
    ...
    if (zoomMode) {
      const brush = d3.brush()
        .extent([[0, 0], [innerWidth, innerHeight]])
        .on("end", ({ selection }) => {
          if (selection) setZoomExtent(selection);
        });
